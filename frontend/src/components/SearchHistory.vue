<template>
  ...
  methods: {
    ...
    async fetchHistory(page = 1) {
      ...
      try {
        const response = await fetch(`http://15.165.119.226/history/?page=${page}`, {
          method: 'GET',
          headers: {
   ...
   async fetchSearchResults(query) {
        try {
          ...
          const response = await fetch('http://15.165.119.226/search/', {
            method: 'POST',
            headers,
      ...
      async deleteHistory(id) {
        const userToken = localStorage.getItem('userToken');
        try {
          const response = await fetch(`http://15.165.119.226/history/delete/${id}`, {
            method: 'DELETE',
            headers: {
              'Authorization': `Bearer ${userToken}`,
              'Content-Type': 'application/json'
