<template>
            ...
            <div class="signin-wrapper">
              <div class="form-wrapper">
                <p class="login-modal-title">Login WIZMAP</p>
                  ...
                  <p>
                    Forgot your 
                    <a id="mobile" @click="showFindIdModal" ><b>id</b></a> 
                    or  
    ...
    <!-- 아이디 찾기 모달 -->
    <div class="modal-wrap" v-show="findIdModalVisible" @click="closeFindIdModal">
        <div class="right-modal-container" @click.stop="">
        <div class="form-wrapper">
            <p class="login-modal-title">Find Your ID</p>
            <input type="email" v-model="findIdEmail" class="form-field" placeholder="Email" />
            <input type="text" v-model="findIdPhone" class="form-field" placeholder="Phone" />
            <button class="button primary" @click="findId">Find ID</button>
            <button class="button secondary" @click="closeFindIdModal">Cancel</button>
            <p v-if="findIdMessage">{{ findIdMessage }}</p>
        </div>
        </div>
    </div>
  ...
  export default {
    data() {
      return {
        ...
        newEmail: '',
        newPhone: '',
        findIdModalVisible: false, // 아이디 찾기 모달 표시 여부
        findIdEmail: '', // 아이디 찾기 이메일
        findIdPhone: '', // 아이디 찾기 휴대폰 번호
        findIdMessage: '' // 아이디 찾기 결과 메시지
      };
    },
    methods: {
      modalOpen() {
      ...
      showFindIdModal() {
      this.findIdModalVisible = true;
    },
    closeFindIdModal() {
      this.findIdModalVisible = false;
    },
    findId() {
      const findIdData = {
      ...
      axios.post('http://localhost:8000/user/find-id/', findIdData)
        ...
        .catch(error => {
          ...
          this.findIdMessage = '일치하는 정보가 없습니다.';
        });
    },
      login() {
    const loginData = {
