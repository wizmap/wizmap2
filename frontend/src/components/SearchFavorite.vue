    ...
    fetchFavoriteData(id) {
      ...
      axios.get(`http://15.165.119.226:8000/favorites`, {
        ...
        .then(response => {
          ...
          return axios.get(`http://15.165.119.226:8000/favorites/like`, {
            headers: {
              'Authorization': `Bearer ${userToken}`
            }
          });
...
updateLocalQuickSlotNameAndIcon(id, newName, newIcon) {
  ...
  likeFavorite(listId) {
    ...
    const userToken = localStorage.getItem('userToken');
    console.log(listId);
    axios.post(`http://15.165.119.226:8000/favorites/list/like/`, {
      list: listId
    }, {
      headers: {
        'Authorization': `Bearer ${userToken}`,
  ...
  unlikeFavorite(listId) {
    ...
    const userToken = localStorage.getItem('userToken');
    console.log(listId);
    axios.delete(`http://15.165.119.226:8000/favorites/list/unlike/${listId}/`, {
      headers: {
        'Authorization': `Bearer ${userToken}`,
        'Content-Type': 'application/json'
      }
